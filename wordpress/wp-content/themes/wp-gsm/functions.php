...
function wpeHeadNav() {
  ...
  array(
    ...
    'link_before'     => '',
    'link_after'      => '',
    'items_wrap'      => '<ul class="ul_one">%3$s</ul>',
    'depth'           => 0,
    'walker'          => ''
    )
  );
...
function easy_breadcrumbs() {
  ...
  if (is_home() || is_front_page()) {
    ...
    if ($show_on_home == 1) echo '<div class="breadcrumbs stroka_echo"><a href="' . $home_link . '">' . $text['home'] . '</a></div>';

  } else {

    echo '<div class="breadcrumbs stroka_echo" xmlns:v="http://rdf.data-vocabulary.org/#">';
    if ($show_home_link == 1) {
...
function taxonomies_category() {
  ...
  register_taxonomy( 'manufacture', 'products', $args );
}
